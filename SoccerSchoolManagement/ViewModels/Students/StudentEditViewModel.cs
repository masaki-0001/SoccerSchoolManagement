...
public class StudentEditViewModel : IValidatableObject
{
    ...
    public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        {
            yield return new ValidationResult("生年月日に未来日は指定できません。", new[] { nameof(BirthDate) });
        }

        if (!string.IsNullOrWhiteSpace(Grade) && !StudentFormOptions.Grades.Contains(Grade))
        {
            yield return new ValidationResult("学年の値が正しくありません。", new[] { nameof(Grade) });
        }

        if (!string.IsNullOrWhiteSpace(Gender) && !StudentFormOptions.Genders.Contains(Gender))
        {
            yield return new ValidationResult("性別の値が正しくありません。", new[] { nameof(Gender) });
        }

        if (!string.IsNullOrWhiteSpace(Status) && !StudentFormOptions.Statuses.Contains(Status))
        {
            yield return new ValidationResult("在籍状況の値が正しくありません。", new[] { nameof(Status) });
        }
